...
var cssProp = (function(prop){
    var b = document.body, added = false;

    var bStyle = getComputedStyle(b, null),
    ...
    for(var i = 0; i < len; i++) {
        ...
        if(bStyle[prefixes[i] +  property] == undefined){
            //b.innerHTML += '<p class=\'unsupported\'>' + prefixes[i] + property + 'Property: ' + bStyle[prefixes[i] +  property + 'Property'] + '</p>' + '<p class=\'unsupported\'>' + prefixes[i] + property + 'Duration: ' + bStyle[prefixes[i] +  property + 'Duration'] + '</p>';
        }
        else {
            // b.innerHTML += '<p>' + prefixes[i] + property + 'Property: ' + bStyle[prefixes[i] +  property + 'Property'] + '</p>' + '<p>' + prefixes[i] + property + 'Duration: ' + bStyle[prefixes[i] +  property + 'Duration'] + '</p>';
...
function scrollToY(scrollTargetY, speed, easing, callback) {
    ...
    if(typeof scrollTargetY == "object") {
        scrollTargetY = scrollTargetY.offset().top;
    };
...
function prepareTmpl(data, tmpl) {
    var key, html;
    ...
    var notRequired = html.match(/({{\?)\w+(}})/g);
    if(notRequired) {
        notRequired = notRequired.filter(onlyUnique);
    }

    for(var i in notRequired) {
        var k = notRequired[i].replace(/([\{,\?,}])/g, "");

        if(typeof data[k] === "undefined" || data[k] === "") {
            html = html.replace(new RegExp('{{\\?'+k+'}}.*{{\\?'+k+'}}', 'm'), "");
        }else{
...
})( this );;// jQuery Mask Plugin v1.14.11
